			...
			validation: {
				'quantity' : {
					fn: function quantityValidator (new_quantity)
					{
						...
						,	minimum_quantity = this.getItem().get('_minimumQuantity')
						,	item_quantity_in_cart = this._getItemQuantityInCart();

						if (!_.isNumber(current_quantity) || _.isNaN(current_quantity) || current_quantity < 1)
						{
							return _.translate('Invalid quantity value');
						}
						else if (item_quantity_in_cart + current_quantity < minimum_quantity)
						{
							return _.translate('Please add $(0) or more of this item', minimum_quantity);
						}
						else if(maximum_quantity && item_quantity_in_cart + current_quantity > maximum_quantity)
						{
							return _.translate('Please add $(0) or less of this item', maximum_quantity - item_quantity_in_cart);
						}
					}
				}
			,	'options': {
		...
		,	_getItemQuantityInCart: function _getItemQuantityInCart()
			{
				...
				,	lines = liveorder.get('lines')
				,	qty = 0
				,	current_item_id = this.getItem().id;
