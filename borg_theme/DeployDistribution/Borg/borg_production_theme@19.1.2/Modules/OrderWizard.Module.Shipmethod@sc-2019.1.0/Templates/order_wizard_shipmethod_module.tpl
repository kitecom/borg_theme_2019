		...
		{{else}}
			{{#if hasShippingMethods}}
				...
				{{else}}
					...
					{{/each}}
					{{#if model.summary.handlingcost}}
						<p class="order-wizard-shipmethod-module-handling">
							<span class="order-wizard-shipmethod-module-handling-cost">
								{{translate 'All delivery options include'}}</span> {{model.summary.handlingcost_formatted}} <span> {{translate 'ADR Dangerous Goods surcharge'}}
							</span>
