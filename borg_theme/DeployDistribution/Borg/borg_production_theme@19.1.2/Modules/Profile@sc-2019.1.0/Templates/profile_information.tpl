...
<h2 class="profile-information-header">{{pageHeader}} {{firstName}}</h2>
		...
		<form class="contact_info">
			...
			<div class="profile-information-form-actions">
				<button type="submit" class="profile-information-button-update">{{translate 'Update'}}</button>
					...
					{{#if hasSalesrep}}
						<small class="profile-rep">{{translate 'For immediate assistance contact <strong>$(0)</strong> at <strong>$(1)</strong>. For additional information, send an email to <strong>$(2)</strong>.' salesrepName salesrepPhone salesrepEmail}}</small>
					{{/if}}
				</div>
		</form>
	</div>
