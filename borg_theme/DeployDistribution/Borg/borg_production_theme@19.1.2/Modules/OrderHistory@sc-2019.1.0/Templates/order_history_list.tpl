...
<section class="order-history-list">
	...
	<div class="order-history-list-header-nav">
		<div class="order-history-list-header-button-group">
			{{#if openIsActive}}
				<span class="order-history-list-header-button-open-active">{{translate 'Open Orders'}}</span>
			{{else}}
				<a href="/open-purchases" class="order-history-list-header-button-open">{{translate 'Open Orders'}}</a>
			{{/if}}
			...
			{{#if allIsActive}}
				<span class="order-history-list-header-button-all-active">{{translate 'All Orders'}}</span>
			{{else}}
				<a href="/purchases" class="order-history-list-header-button-all">{{translate 'All Orders'}}</a>
			{{/if}}
		</div>
		<div data-view="ListHeader" class="listHeader" {{#if openIsActive}}style="display:none;"{{/if}}></div>
	</div>
	...
	{{else}}
		...
		{{else}}
			<div class="order-history-list-empty-section">
				<h5>{{translate 'You don\'t have any orders in your account right now.'}}</h5>

				{{#unless allIsActive}}
					<p>{{translate 'To see a list of all your past orders, you can go to the tab <a href="/purchases" class="">All</a>.'}}</p>
				{{/unless}}
