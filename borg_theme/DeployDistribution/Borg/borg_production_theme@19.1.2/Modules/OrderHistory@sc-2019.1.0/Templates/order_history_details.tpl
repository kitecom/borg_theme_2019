...
<section>
	<header>
		<h2 class="order-history-details-order-title" data-origin='{{originName}}'>
			...
			<span class="order-history-details-total-formatted">
				Total: {{model.summary.total_formatted}}
			</span>
		</h2>
	</header>
	...
	{{#if showPaymentEventFail}}
		<div class="order-history-details-message-warning">
			{{translate 'The checkout process for this order was not completed. To place the order, please <a data-navigation="ignore-click" href="$(0)" >finalize your payment.</a>' model.paymentevent.redirecturl}}
		</div>
	{{/if}}
		...
		<div class="order-history-details-header-row">
			<div class="order-history-details-header-col-left">
				<p class="order-history-details-header-date-info">
					{{translate '<span class="order-history-details-header-date-info-date-label"><strong>Date:</strong> </span> <span class="order-history-details-header-date">$(0)</span>' model.trandate}}
				</p>
				

				{{#if showPurchaseOrderNumber}}
					<p class="order-history-details-header-purchase-order-number-info">
						{{translate '<span class="order-history-details-header-purchase-order-info-purchase-order-number-label"><strong>PO#</strong> </span> <span class="order-history-details-header-purchase-order-number">$(0)</span>' model.purchasenumber}}
					</p>
				{{/if}}
				<p class="order-history-details-header-atp-info">
					<strong>Ship Date:</strong>{{model.options.custbody_bbs_otif_date}}
				</p>
					<p class="order-history-details-header-opb-info">
					<strong>Order Placed by:</strong> {{model.options.custbody_bbs_order_contact_name}} {{model.options.custbody_cibraised_by}}
				</p>
					...
					<div class="order-history-details-accordion-head">
						<a class="order-history-details-accordion-head-toggle-secondary collapsed" data-toggle="collapse" data-target="#products-not-shipp" aria-expanded="true" aria-controls="products-not-shipp">
							{{#if nonShippableItemsLengthGreaterThan1}}
								{{translate 'Non Shipping Items ($(0))' nonShippableLines.length}}
							{{else}}
								{{translate 'Non Shipping Item ($(0))' nonShippableLines.length}}
							{{/if}}
