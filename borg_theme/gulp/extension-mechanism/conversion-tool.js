...
,	initializeExtensionFolder: function initializeExtensionFolder(manifest)
	{
		...
		switch(manifest.type)
		{
			...
			case 'extension':
				...
				{
					folder_path = path.join(nconf.get('folders:source:extras_path'), manifest.vendor, manifest.name);
				}
				break;
		}
...
,	extensionToModules: function extensionToModules(manifest, files)
	{
		...
		{	
			...
			if(resource)
			{
				...
				{
					var override_destination_path = path.join(nconf.get('folders:overrides_path'), manifest.vendor, manifest.name, path.dirname(local_file_destination));
