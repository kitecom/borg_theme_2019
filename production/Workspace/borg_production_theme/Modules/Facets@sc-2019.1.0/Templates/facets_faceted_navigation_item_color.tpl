		...
		{{/if}}
			<div class="facets-faceted-navigation-item-color-facet-group-content">
				<ul class="facets-faceted-navigation-item-color-picker">
					{{#each displayValues}}
						<li >
							<a id="{{color}}" href="{{link}}" title="{{label}}" data-color="{{label}}" class="facets-faceted-navigation-item-color-option {{#if isLightColor}}white-border{{/if}} {{#if isActive}}active{{/if}}">
								{{#if isImageTile}}
									<img
										src="{{image.src}}"
										alt="{{label}}"
