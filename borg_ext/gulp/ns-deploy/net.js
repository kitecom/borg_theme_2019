...
function getAuthorizationHeader(deploy)
{
	return	'NLAuth nlauth_account=' + deploy.info.account + ', ' +
			...
			'nlauth_signature=' + encodeURIComponent(deploy.info.password) + ', ' +
...
,	roles: function (deploy, cb)
	{
		...
		{
			var host = deploy.options.molecule ?
			...
			var requestUrl = host + '/rest/roles';
			...
			,	{
					...
					,	'Authorization': 'NLAuth nlauth_email=' + deploy.info.email + ', nlauth_signature=' + encodeURIComponent(deploy.info.password)
					,   'User-Agent': deploy.info.user_agent
					}
				,	rejectUnauthorized: false
				}
