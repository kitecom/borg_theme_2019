		...
		_.each(content.overrides, function(val, key) {
			...
			if (override_info.isOverriden)
			{
				error_logger.push(
					...
					' is overridden more than once. Overridden in modules: ', c.cyan(override_info.moduleName), ' and ', c.cyan(module_name));
			}
			...
			{
				error_logger.push(
					'+- Source file: ', c.cyan(original_path),
					' does not exists. Defined in module ', c.cyan(module_name));
			}
			...
			{
				error_logger.push(
					'+- Override file: ', c.cyan('Modules' + override_path.split('Modules')[1]),
					' does not exists. Defined in module ', c.cyan(module_name));
			}
			...
			override_logger.push('+- File: ', c.cyan(shortenModulePath(original_path)), ' overridden with: ', c.cyan(shortenModulePath(override_path)));
		});
	}
		...
		return map(function(file,cb)
		{
			...
			else
			{
				...
				if (override_info.isOverriden)
				{
					...
					file.contents = Buffer.from(fs.readFileSync(override_info.overridePath));
				}
